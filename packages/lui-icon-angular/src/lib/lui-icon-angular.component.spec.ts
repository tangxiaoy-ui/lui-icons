...
import { LUI-iconAngularModule } from './lui-icon-angular.module';
import { formatFixed, LUI-iconAngularComponent } from './lui-icon-angular.component';
import defaultAttributes from '../icons/constants/default-attributes';
import { LUI-iconIcons } from '../icons/types';

describe('LUI-iconAngularComponent', () => {
  ...
  const testIcons: LUI-iconIcons = {
    Demo: [['polyline', { points: '1 1 22 22' }]],
  };

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [LUI-iconAngularComponent, TestHostComponent],
      imports: [LUI-iconAngularModule.pick(testIcons)],
    }).compileComponents();
  ...
  it('should add all classes', () => {
    ...
    expect(getAttribute('class')).toBe('lucide lui-icon-demo my-icon');
  });
  ...
  @Component({
    selector: 'lui-icon-spec-host-component',
    template: `<i-lucide
      name="demo"
      class="my-icon"
      [color]="color"
