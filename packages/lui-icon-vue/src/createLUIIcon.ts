...
import { toKebabCase } from '@lui-icon/shared';
...
export default (iconName: string, iconNode: IconNode): Component => ({
  ...
  props: {
    ...
    defaultClass: {
      type: String,
      default: `lui-icon-icon lucide lui-icon-${toKebabCase(iconName).replace('-icon', '')}`,
    },
  },
  render(
    createElement,
    ...
    if (showDeprecationWarning) {
      console.warn(
        '[LUI-icon Vue] This package will be deprecated end of 2023. Please upgrade to Vue 3 and use the latest lucide package for Vue.',
      );
